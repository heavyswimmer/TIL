...
- Italic 체를 지정해주는 요소로, 의미론적 중요성이 없다.
...
- 중요한 부분을 이탤릭체로 강조(emphasize) 하는 시맨틱 요소이다.
...
- 텍스트를 굵게 표현해주는 요소로, 의미론적 중요성이 없다.
...
- 정렬되지 않은 목록(unordered list) 을 나타낼 때 사용하는 요소이다.
- 보통 불릿으로 표현한다.
...
- 정렬된 목록(ordered list) 을 나타내는 요소이다.
- 보통 숫자로 표현한다.
